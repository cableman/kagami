...
function connectCache() {
  if (service === undefined) {
    ...
    service = self.CacheService.createClient(config.port, config.host, { 'auth_pass': config.auth });
    ...
    service.on("connect", function (err) {
      ...
      service.select(config.db, function() {
        console.log('Connected to cache service server at: ' + config.host);
